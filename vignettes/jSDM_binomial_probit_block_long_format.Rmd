...
$$ \mathrm{probit}(\theta_{n}) =\alpha_i + D_n.\beta + X_n.\beta_{sp.j} +  W_i.\lambda_j,$$
...
- Latent variable $z_{n} = \alpha_i + D_n.\beta + X_n.\beta_j + W_i.\lambda_j + \epsilon_{n}$, with $\forall (n) \ \epsilon_{n} \sim \mathcal{N}(0,1)$ and such that:
...
- Explanatory variables: bioclimatic data about each site. $X=(X_i)_{i=1,\ldots,nobs}$ with $X_i=(1,x_i^1,\ldots,x_i^p)\in \mathbb{R}^{(p+1)}$ where $p$ is the number of bioclimatic variables considered.
...
- $\beta$ correspond regression coefficients of explanatory variables found in matrix $D=(D_n)_{i=1,\ldots,nobs}$. We use a prior distribution $\mathcal{N}(0,10)$ for all betas. 
...
```{r data-set}
data<- data.frame(site=rep(rownames(pres_data), nsp), species=rep(colnames(pres_data), each=nsite))
...
We use the `jSDM_binomial_probit_block_long_format()` function to fit the jSDM (increase the number of iterations to achieve convergence). 

```{r jSDM-probit}
mod <- jSDM::jSDM_binomial_probit_block_long_format(
  # Chains
  burnin=1000, mcmc=1000, thin=1,
  # Response variable 
  data=data, 
  # Explanatory variables 
  site_suitability = ~ snow + snow2 + snow.SLA + (snow + snow2):species,   
  # Model specification 
...
np <- nrow(mod$model_spec$beta_sp_start)
nd <- length(mod$model_spec$beta_start)
## beta of the first two species
plot(mod$mcmc.beta)

## beta_sp.j of the first two species
par(mfrow=c(np,2), oma=c(0,0,2,0))
for (j in 1:2) {
    plot(mod$mcmc.sp[[paste0("sp_",j)]][,1:np])
...
for (j in 1:2) {
    plot(mod$mcmc.sp[[paste0("sp_",j)]][,(np+1):(np+n_latent)])
    title(outer=TRUE, main=paste0( "species ", j ," : ",
                                   unique(data$species)[j]), cex.main=1.5)
}
...
## probit_theta
par (mfrow=c(2,1))
hist(mod$probit_theta_pred,
     main = "Predicted probit theta", xlab ="predicted probit theta")
hist(pnorm(mod$probit_theta_pred),
...
simdata <- data.frame(site=Id_sites, species=Id_species, row.names=1:nobs)
