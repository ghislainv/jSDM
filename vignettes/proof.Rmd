...
\Rightarrow  
\begin{cases}
y_{i,j}| z_{i,j} \sim \mathcal{B}ernouilli(\theta_{i,j}) \text{ with } \\
...
# Proposition about conjugate prior used :

- We can bring us back to a models of the form : $Z' = X\beta +\epsilon$ to estimate the posterior distributions of betas, lambdas and latent variables $W_i$ of the model. For example concerning latent varaible $\lambda_j$, we define $Z'_{i,j} = Z_{i,j} - \alpha_i - \beta_{0j} - X_i'\beta_j$ such as $Z'_{i,j} = W_i'\lambda_j +\epsilon_{i,j}$ so $Z'_{i,j} \ | \ W_i \ , \ \lambda_j \  \sim \mathcal{N}( W_i'\lambda_j,1)$.  
...
- About posterior distribution of random site effect $(\alpha_i)_{i=1,\dots,nsite}$, we can use model of the form $Z'_{i,j} =\alpha_i +\epsilon_{i,j}$, with $Z'_{i,j} = Z_{i,j} - W_i'\lambda_j - X_i'\beta_j- \beta_{0j}$ so $Z'_{i,j} \ | \ W_i \ , \ \lambda_j, \ \beta_j, \ \beta_{0j}, \ \alpha_i \ \sim \mathcal{N}(\alpha_i,1)$. We use the following proposition :  
If $$\begin{cases} 
...
\end{cases}
\begin{cases} 
\Rightarrow \theta | \ x &\sim \mathcal{N}(\mu_1,{\tau_1}^2) \text{ with }\\
...
- Concerning posterior distribution of $V_{\alpha}$, the variance of random site effect $(\alpha_i)_{i=1,\dots,nsite}$, we use the following proposition :   
If $$\begin{cases} 
