...
$$ \mathrm{probit}(\theta_{i,j}) =\alpha_i + \beta_{0j}+X_i'\beta_j+ W_i'\lambda_j + \epsilon_{i,j}$$
...
It can be easily shown that: $y_{i,j}| Z_i \sim \mathcal{B}ernouilli(\theta_{i,j})$. 
...
We assume that $W_i \sim \mathcal{N}(0,I_q)$ and we define the associated coefficients: $\lambda_j=(\lambda_j^1,\ldots, \lambda_j^q)'$.
...
- $\beta_{0j}$ correspond to the intercept for species $j$ which is assume to be a fixed effect.

- $\alpha_i$ represents the random effect of site $i$. 


# Data-set

(ref:cap-frog) **Frog**. @Wilkinson2019.

```{r frog-picture, echo=FALSE, out.width="\\textwidth", fig.cap="(ref:cap-frog)"}
knitr::include_graphics("figures/frog.jpg")
```
...
```{r plot-results}
## alpha_i of 4 first sites
plot(mod_jSDM_block_frogs$mcmc.alpha[,1:4])
...
## Latent variables W_i of 4 first sites
par(mfrow=c(1,2))
for (l in 1:n_latent) {
  plot(mod_jSDM_block_frogs$mcmc.latent[[paste0("lv_",l)]][,1:4], main = paste0("Latent variable W_", l))
}
...
## Deviance
par (mfrow=c(1,1))
hist(mod_jSDM_block_frogs$probit_theta_pred, main = "Predicted probit theta")
