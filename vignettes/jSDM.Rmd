...
In this vignette, we illustrate the use of the `jSDM` R package wich aims at providing user-friendly statistical functions using field observations (occurence or abundance data and various explanatory variables) to fit jSDMs models.
...
```{r jSDM, cache=TRUE}
T1<- Sys.time()
mod_frogs_jSDM_logit <- jSDM_binomial_logit(
  # Chains
  burnin=100, mcmc=100, thin=1,
  # Response variable 
  presence_site_sp = PA_frogs, 
...
```{r plot-results}
## alpha_i of the first two sites
plot(coda::as.mcmc(mod_frogs_jSDM_logit$mcmc.alpha[,1:2]))

## V_alpha
par(mfrow=c(1,2))
coda::traceplot(mod_frogs_jSDM_logit$mcmc.V_alpha, main="V_alpha")
coda::densplot(mod_frogs_jSDM_logit$mcmc.V_alpha, main="V_alpha")

np <- nrow(mod_frogs_jSDM_logit$model_spec$beta_start)

## beta_j of the first two species
par(mfrow=c(np,2))
for (j in 1:2) {
  for (p in 1:np) {
      coda::traceplot(coda::as.mcmc(mod_frogs_jSDM_logit$mcmc.sp[[paste0("sp_",j)]][,p]))
      coda::densplot(coda::as.mcmc(mod_frogs_jSDM_logit$mcmc.sp[[paste0("sp_",j)]][,p]), 
main = paste(colnames(mod_frogs_jSDM_logit$mcmc.sp[[paste0("sp_",j)]])[p],
", species : ",j))
  }
}

## lambda_j of the first two species
n_latent <- mod_frogs_jSDM_logit$model_spec$n_latent
par(mfrow=c(n_latent*2,2))
for (j in 1:2) {
  for (l in 1:n_latent) {
      coda::traceplot(coda::as.mcmc(mod_frogs_jSDM_logit$mcmc.sp[[paste0("sp_",j)]][,np+l]))
      coda::densplot(coda::as.mcmc(mod_frogs_jSDM_logit$mcmc.sp[[paste0("sp_",j)]][,np+l]), 
      main = paste(colnames(mod_frogs_jSDM_logit$mcmc.sp[[paste0("sp_",j)]])[np+l],
      ", species : ",j))
    }
  }
...
par(mfrow=c(1,2))
for (l in 1:n_latent) {
  plot(mod_frogs_jSDM_logit$mcmc.latent[[paste0("lv_",l)]][,1:2], main = paste0("Latent variable W_", l))
}

## probit_theta
par (mfrow=c(1,1))
hist(mod_frogs_jSDM_logit$probit_theta_pred, main = "Predicted probit theta", xlab ="predicted probit theta")

## Deviance
plot(mod_frogs_jSDM_logit$mcmc.Deviance,main = "Deviance")
...
```{r correlation-matrix}
plot_residual_cor(mod_frogs_jSDM_logit)
```

## Predictions 

We use the `predict()` S3 method on the `mod_frogs_jSDM_logit` object of class `jSDM` to compute the mean (or expectation) of the posterior distributions obtained and get the expected values of model's parameters.
...
Id_species <- colnames(PA_frogs)
# Simulate new observations of covariates on those sites 
simdata <- matrix(nrow=50, ncol = ncol(mod_frogs_jSDM_logit$model_spec$site_data))
colnames(simdata) <- colnames(mod_frogs_jSDM_logit$model_spec$site_data)
rownames(simdata) <- Id_sites
simdata <- as.data.frame(simdata)
simdata$Covariate_1 <- rnorm(50)
simdata$Covariate_3 <- rnorm(50)
...
# Predictions 
theta_pred <- predict(mod_frogs_jSDM_logit, newdata=simdata, Id_species=Id_species,
						   Id_sites=Id_sites, type="mean")
hist(theta_pred, main="Predicted theta with simulated data", xlab="predicted theta")
```
