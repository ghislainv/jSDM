...
`jSDM-package` is an R package for fitting joint species distribution models (jSDM) in a hierarchical Bayesian framework.
...
In this vignette, we illustrate the use of the `jSDM-package` wich aims at providing user-friendly statistical functions using field observations (occurence or abundance data) to fit jSDMs models.
...
Model types available in `jSDM-package` are not limited to those described in this example. `jSDM` includes various model types for occurrence and abundance data: 
...
Below, we show an example of the use of ['jSDM'][jSDM-package] R package for fitting species distribution model to occurence data for 9 frog's species.
...
We use the [`jSDM_binomial_probit_block()`][jSDM_binomial_probit_block()]
...
We use the function [`plot_residual_cor()`][plot_residual_cor()] to compute and display the residual correlation matrix between species :
...
We use the [`predict.jSDM()`][predict.jSDM()] S3 method on the `mod_frogs_jSDM_probit` object of class `jSDM` to compute the mean (or expectation) of the posterior distributions obtained and get the expected values of model's parameters.
