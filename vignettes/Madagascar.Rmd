...
```{r plot-associations2, echo=FALSE, out.width=800, out.height=800, eval=TRUE}
...
# Take same sample of 10000 cells than present presence probabilities raster
npart <- 10
samp_theta <- NULL
for(n in 1:npart){
  theta <- stack(paste0("~/Documents/projet_BioSceneMada/Report/data/RST_theta_future_", n, ".tif"))
...
# Compute PCA on sampled theta
library(ade4)
pca_theta <- dudi.pca(theta_df, center=TRUE, scale=TRUE, nf=3, scannf = FALSE)
save(pca_theta, file="~/Documents/projet_BioSceneMada/Report/data/PCA_theta_future.RData")
str(pca_theta)
...
```{r plot-diversity-beta, eval=TRUE}
