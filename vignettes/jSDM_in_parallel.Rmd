...
mcmc_list_lambda <- mcmc.list(lapply(mcmc_list_param[,grep("lambda", colnames(mcmc_list_param[[1]]), value=TRUE)], arr2mcmc))
mcmc_list_centered_lambda <- mcmc.list(lapply(lapply(mcmc_list_lambda, scale, scale=FALSE),arr2mcmc))
mcmc_list_deviance <- mcmc.list(lapply(lapply(mod_probit,"[[","mcmc.Deviance"), arr2mcmc))
...
psrf_lv <- mean(gelman.diag(mcmc_list_lv,
                            multivariate=FALSE)$psrf[,2 ])
psrf_lambda_centered <- mean(gelman.diag(mcmc_list_centered_lambda,
                                multivariate=FALSE)$psrf[,2], na.rm=TRUE)
psrf_lv_centered <- mean(gelman.diag(mcmc_list_centered_lv,
                            multivariate=FALSE)$psrf[,2 ])
save(psrf_lambda, psrf_lv, psrf_alpha, psrf_V_alpha, psrf_beta,
     file="jSDM_in_parallel_cache/psrf.rda")

Rhat <- data.frame(Rhat=c(psrf_alpha, psrf_V_alpha, psrf_beta, psrf_lambda, psrf_lambda_centered, psrf_lv, psrf_lv_centered),
                   Variable=c("alpha", "Valpha", "beta", "lambda", "centered lambda", "W", "centered W"))
# Barplot
library(ggplot2)
...
However the $\hat{R}$ obtained for latent variables $W$ and factor loadings $\lambda$ are far from 1. It can be explained by the fact that $W$ and $\lambda$ can takes alternately positive and negative values between and even within MCMC chains, due to the identifiability issue between $W$ and $\lambda$. 
Consequently, if we center the latent variables $W$ and  factor loadings $\lambda$  estimated before computing the $\hat{R}$, we get values well closer to 1, we can therefore consider that convergence has been reached for these parameters. 
