...
Among the following datasets, the presence-absence data are from the [@Wilkinson2019] article in which they are used to compare joint species distribution models for presence-absence data, the dataset that records the presence or absence of `birds` during several visits to each site is from the [@Kery2006] article and the `mites` abundance data-set is from the [Borcard1994] article.
...
datasets["n.site",] <- c(300,nrow(mosquitos),nrow(eucalypts), nrow(frogs), nrow(fungi), nrow(birds), nrow(mites))
...
In a first step, we fit joint species distribution models from previous datasets using the `boral()` function from package of the same name  whose functionalities are developed in the article [@Hui2016]. 
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the simulated dataset using the `boral()` function to perform binomial probit regression.
...
## Prediction
# probit_theta_pred 
plot(probit_theta,probit_theta_pred_sim,
     main="probit(theta)", xlab ="obs", ylab="fitted")
abline(a=0,b=1,col='red')
```


```{r boral-simulation-plot2, echo=FALSE, out.width=800, eval=T}
knitr::include_graphics("jSDM_boral_files/figure-html/boral-simulation-plot-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/boral-simulation-plot-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/boral-simulation-plot-3.png")
knitr::include_graphics("jSDM_boral_files/figure-html/boral-simulation-plot-4.png")
```

## Mosquitos dataset

We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`mosquitos`](https://ecology.ghislainv.fr/jSDM/reference/mosquitos.html) dataset using `boral()` function to perform binomial probit regression.
...
mod_boral_Mosquitos <- boral(y=PA_Mosquitos, X=Env_Mosquitos,
                             ...
                             row.eff="random", hypparams=c(100,10,100,30),
                             mcmc.control=list(n.burnin=10000,n.iteration=15000,
                                               n.thin=5,seed=123))
T2 <- Sys.time()
...
# Predicted probit(theta) 
probit_theta_pred_Mosquitos <- mod_boral_Mosquitos$row.coefs[[1]]$mean +
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`eucalypts`](https://ecology.ghislainv.fr/jSDM/reference/eucalypts.html) dataset using `boral()` function to perform binomial probit regression.
...
# Predicted probit(theta) 
probit_theta_pred_Eucalypts <- mod_boral_Eucalypts$row.coefs[[1]]$mean + 
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`frogs`](https://ecology.ghislainv.fr/jSDM/reference/frogs.html) dataset using `boral()` function to perform binomial probit regression.
...
# Predicted probit(theta) 
probit_theta_pred_Frogs <- mod_boral_Frogs$row.coefs[[1]]$mean + 
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`fungi`](https://ecology.ghislainv.fr/jSDM/reference/fungi.html) dataset using `boral()` function to perform binomial probit regression.
...
# Predicted probit(theta) 
probit_theta_pred_Fungi <- mod_boral_Fungi$row.coefs[[1]]$mean +
...
We fit a binomial joint species distribution model with multiple visit by site, including random site effect and latent variables, from the [`birds`](https://ecology.ghislainv.fr/jSDM/reference/birds.html) dataset using `boral()` function to perform binomial logistic regression.

We have to specify the `JAGS` model in the `jagsboralmodel.txt` file because the default `JAGS` model generated by `boral` function doesn't allow to indicate different number of trials for each site as it is the case in [`birds`](https://ecology.ghislainv.fr/jSDM/reference/birds.html) dataset. 
...
#                          mcmc.control=list(n.burnin=10000, n.iteration=15000,
#                                            n.thin=5,seed=123))
# JAGS model in vignettes/jagsboralmodel.txt modified to specify the number of trials by site
...
mod_boral_Birds <- jagsUI::jags(data = jags.data, inits <- gen.inits,
                                ...
                                model.file = "jagsboralmodel.txt",
                                n.chains = 1, n.iter=15000, n.burnin = 10000, n.thin = 5)
T2 <- Sys.time()
T_boral_Birds <- difftime(T2,T1)

# Predicted probit(theta) 
logit_theta_pred_Birds <- c(mod_boral_Birds$mean$row.coefs) +
...
for (i in 1:nrow(PA_Birds)){
  for (j in 1:ncol(PA_Birds)){
    theta <- inv_logit(logit_theta_pred_Birds[i,j])
    logL= logL + dbinom(PA_Birds[i,j],Env_Birds$nsurvey[i],theta,1)  
  }
}
Deviance_boral_Birds <- -2*logL
# mod_boral_Birds$mean$deviance
save(T_boral_Birds, mod_boral_Birds,
     logit_theta_pred_Birds, Deviance_boral_Birds,
     file="jSDM_boral_cache/boral_Birds.RData")
```

## Mites dataset

We fit a joint species distribution model, including random site effect and latent variables, from the [`mites`](https://ecology.ghislainv.fr/jSDM/reference/mites.html) abundance dataset using `boral()` function to perform a poisson log-linear regression.
...
# Predicted probit(theta) 
log_theta_pred_Mites <- as.matrix(X_Mites[,-1]) %*% t(mod_boral_Mites$X.coefs.mean) +
...
save(T_boral_Mites, mod_boral_Mites,
     ...
     file="~/Documents/jSDM/vignettes/jSDM_boral_cache/boral_Mites.RData")
```
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the simulated dataset using the `jSDM_binomial_probit_block()` function to perform binomial probit regression.
...
## probit_theta
par(mfrow=c(1,2))
plot(probit_theta,mod_jSDM_sim$probit_theta_pred,
     xlab="obs",ylab="fitted", main="probit(theta)")
abline(a=0,b=1,col='red')
## Z
plot(Z_true,mod_jSDM_sim$Z_latent,
     xlab="obs", ylab="fitted", main="latent variable Z")
abline(a=0,b=1,col='red')
```


```{r jSDM-simulation-plot2, echo=FALSE, out.width=800, eval=T}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-3.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-4.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-5.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-6.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-7.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-8.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-9.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-simulation-plot-10.png")

```

## Mosquitos dataset

We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`mosquitos`](https://ecology.ghislainv.fr/jSDM/reference/mosquitos.html) dataset using `jSDM_binomial_probit_block()` function to perform binomial probit regression.
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`eucalypts`](https://ecology.ghislainv.fr/jSDM/reference/eucalypts.html) dataset using `jSDM_binomial_probit_block()` function to perform binomial probit regression.
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`frogs`](https://ecology.ghislainv.fr/jSDM/reference/frogs.html) dataset using `jSDM_binomial_probit_block()` function to perform binomial probit regression.
...
We fit a binomial joint species distribution model, including random site effect and latent variables, from the [`fungi`](https://ecology.ghislainv.fr/jSDM/reference/fungi.html) dataset using `jSDM_binomial_probit_block()` function to perform binomial probit regression.
...
for (i in 1:nsite){
  for (j in 1:nsp){
    theta <- pnorm(mod_jSDM_sim$probit_theta_pred[i,j])
    logL=logL + dbinom(Y[i,j],1,theta,1)  
  }
}
...
```{r jSDM-boral-simulation-plot, echo=FALSE, eval=T, out.width=800}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-simulation-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-simulation-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-simulation-3.png")
```
...
```{r jSDM-boral-mosquitos-plot, echo=FALSE, eval=T, out.width=800}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-mosquitos-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-mosquitos-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-mosquitos-3.png")
```
...
```{r jSDM-boral-Eucalypts-plot, echo=FALSE, eval=T, out.width=800}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-Eucalypts-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-Eucalypts-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-Eucalypts-3.png")
```
...
```{r jSDM-boral-frogs-plot, echo=FALSE, out.width=800, eval=T}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-frogs-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-frogs-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-frogs-3.png")
```
...
```{r jSDM-boral-fungi-plot, echo=FALSE, out.width=800, eval=T}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-fungi-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-fungi-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-fungi-3.png")
```
...
# Predictions 
par(mfrow=c(1,1))
plot(logit_theta_pred_Birds, logit(mod_jSDM_Birds$theta_latent), xlab="fitted by boral", ylab="fitted by jSDM", main="logit(theta)")
abline(a=0,b=1,col='red')
```


```{r jSDM-boral-birds-plot, echo=FALSE, out.width=800, eval=T}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-birds-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-birds-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-birds-3.png")
```
...
# Predictions 
par(mfrow=c(1,1))
plot(log_theta_pred_Mites, apply(mod_jSDM_Mites$theta_latent,c(1,2),log), xlab="fitted by boral", ylab="fitted by jSDM", main="log(theta)")
abline(a=0,b=1,col='red')
```


```{r jSDM-boral-mites-plot, echo=FALSE, out.width=800,eval=T}
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-mites-1.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-mites-2.png")
knitr::include_graphics("jSDM_boral_files/figure-html/jSDM-boral-mites-3.png")
```
