...
#' mcmc.alpha \tab An mcmc object that contains the posterior samples for site effects \eqn{\alpha}, not returned if \code{site_effect="none"}.\cr
...
#' mcmc.sp \tab A list by species of mcmc objects that contains the posterior samples for species effects \eqn{\beta_j} and \eqn{\lambda_j} if \code{n_latent>0} with \eqn{j=1,\ldots,n_{species}}{j=1,...,n_species}.\cr
...
#'  if \code{n_latent=0} and \code{site_effect="fixed"} \tab probit\eqn{(\theta_{ij}) = \beta_{0j} + X_i \beta_j  + \alpha_i}{(\theta_ij) = \beta_0j + X_i \beta_j + \alpha_i} \cr
...
#' follows the same a priori Gaussian distribution such that \eqn{\beta_j \sim \mathcal{N}_{np}(\mu_{\beta},V_{\beta})}{\beta_j ~ N_np(\mu_\beta,V_\beta)},
#' for each species. 
#' 
#' If species traits data are provided, the effect of species \eqn{j}: \eqn{\beta_j};
#' follows an a priori Gaussian distribution such that \eqn{\beta_j \sim \mathcal{N}_{np}(\mu_{\beta_j},V_{\beta})}{\beta_j ~ N_np(\mu_\betaj,V_\beta)},
...
{   
  ...
  if(is.null(trait_data)){
    ...
    if(n_latent==0 && site_effect=="none"){
      ...
      # call Rcpp function
      mod <- Rcpp_jSDM_binomial_probit(ngibbs=ngibbs, nthin=nthin, nburn=nburn,
                                       Y=Y, X=as.matrix(X),
      ...
      model_spec <- list(burnin=burnin, mcmc=mcmc, thin=thin,
                         presence_data=Y, n_latent=n_latent, 
    ...
    if(n_latent>0 && site_effect=="none"){
      ...
      #= Model specification, site_formula,
      model_spec <- list(presence_data=Y,
                         site_formula=site_formula,
  ...
  if(!is.null(trait_data)){
    ...
    if(n_latent>0 && site_effect=="none"){
      ...
      # call Rcpp function
      mod <- Rcpp_jSDM_binomial_probit_traits_lv(ngibbs=ngibbs, nthin=nthin, nburn=nburn,
                                                 Y=Y, X=as.matrix(X), Tr=as.matrix(Tr),
    ...
    if(n_latent==0 && site_effect=="fixed"){
      ...
      # call Rcpp function
      mod <- Rcpp_jSDM_binomial_probit_traits_fixed_site(ngibbs=ngibbs, nthin=nthin, nburn=nburn,
                                                         Y=Y, X=as.matrix(X), Tr=as.matrix(Tr),
                                                         beta_start=beta_start,
                                                         V_beta=Vbeta, 
                                                         alpha_start=alpha_start, V_alpha=V_alpha,
    ...
    if(n_latent>0 && site_effect=="fixed"){
      ...
      # call Rcpp function
      mod <- Rcpp_jSDM_binomial_probit_traits_fixed_site_lv(ngibbs=ngibbs, nthin=nthin, nburn=nburn,
                                                            Y=Y, X=as.matrix(X), Tr=as.matrix(Tr),
                                                            beta_start=beta_start, V_beta=Vbeta,
                                                            lambda_start=lambda_start, V_lambda=Vlambda, mu_lambda=mulambda,
                                                            W_start=W_start, V_W=V_W,
