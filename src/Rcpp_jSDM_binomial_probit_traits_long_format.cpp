...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_binomial_probit_traits_long_format(
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++) {
      // small_v
      arma::vec small_v = inv_Vbeta*mu_beta+ X.rows(rowId_sp[j]).t()*(Z_run(rowId_sp[j])-D.rows(rowId_sp[j])*gamma_run);
    ...
    for ( int n = 0; n < NOBS; n++ ) {
      // probit(theta_ij) = X_i*beta_j + D_ij*gamma
      probit_theta_run(n) = arma::as_scalar(X.row(n)*beta_run.col(Id_sp(n))+D.row(n)*gamma_run);
...
} // end Rcpp_jSDM_binomial_probit_traits_long_format
...
# # Result analysis
# # ===================================================
# pdf(file="~/Documents/jSDM.test/results_binomial_probit_traits_long_format.pdf")
# # Parameter estimates
# par(mfrow=c(1,2))
# hist(probit_theta)
# # ## theta
...
# glm <- glm( Y ~  x1.SLA + species + x1:species + x1.2:species -1, family=binomial(link="probit"), data=data)
# par(mfrow=c(1,2))
...
# segments(x0 = x0s, x1 = x1s, y0=glm$coefficients[c("x1.SLA")], col = "blue", lwd=2)
# legend("topright", legend=c("gamma target", "gamma fitted by glm"), col=c('red','blue'), lty=1, lwd=2)
# dev.off()
*/
