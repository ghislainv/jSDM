...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_binomial_probit_traits_rand_site_long_format(
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int i=0; i<NSITE; i++) {
      ...
      for (int n=0; n<nobs_i; n++) {
        small_v2 += arma::as_scalar(Z_run.row(rowId_site[i].at(n))-X.row(rowId_site[i].at(n))*beta_run.col(Id_sp(rowId_site[i].at(n)))-D.row(rowId_site[i].at(n))*gamma_run);
      }
      // big_V
      double big_V2 = 1.0/(1.0/V_alpha_run + nobs_i);
...
} // end Rcpp_jSDM_binomial_probit_traits_rand_site_long_format
...
# # probit_theta_supObs <- c(X_supObs%*%beta.target) + as.matrix(D_supObs[,c("x1.2", x1.SLA")]) %*% gamma.target
...
# ngibbs <- nsamp+nburn
# T1 <- Sys.time()
# mod <- Rcpp_jSDM_binomial_probit_traits_rand_site_long_format(
...
# # Result analysis
# # ===================================================
# pdf(file="~/Documents/jSDM.test/results_binomial_probit_traits_rand_site_long_format.pdf")
# # Parameter estimates
# par(mfrow=c(1,2))
# hist(probit_theta)
# ## theta
