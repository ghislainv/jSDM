...
Rcpp::List Rcpp_jSDM_binomial_probit_block_traits_fixed_site_lv_long_format(
  ...
  // probit_theta_ij = X_i*gamma_j + alpha_i + W_i*lambda_j + D_ij*gamma
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++) {
      // small_v
      arma::vec small_v = inv(V_param_sp)*mu_param_sp + data.rows(rowId_sp[j]).t()*(Z_run(rowId_sp[j])- D.rows(rowId_sp[j])*gamma_run-alpha_run(Id_site(rowId_sp[j])));
...
} // end Rcpp_jSDM_binomial_probit_block_traits_fixed_site_lv_long_format
...
# ngibbs <- nsamp+nburn
# T1 <- Sys.time()
# mod <- Rcpp_jSDM_binomial_probit_block_traits_fixed_site_lv_long_format(
...
# # Result analysis
# # ===================================================
# pdf(file="~/Documents/jSDM.test/results_binomial_probit_block_traits_fixed_site_lv_long_format.pdf")
# par(mfrow=c(1,1))
# hist(probit_theta)
# # Parameter estimates
# ## probit_theta
