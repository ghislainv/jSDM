...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_binomial_probit_rand_site_lv(const int ngibbs,const int nthin,const int nburn, 
                                                  const arma::umat &Y, 
                                                  const arma::mat &X,
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++) {
      // small_v
      arma::vec small_v = inv(V_beta)*mu_beta + X.t()*(Z_run.col(j) - W_run*lambda_run.col(j) - alpha_run);
      // big_V
      ...
      // small_v
      small_v = inv(V_lambda)*mu_lambda + W_run.t()*(Z_run.col(j) - X*beta_run.col(j) - alpha_run);
      // big_V
    ...
    for (int i=0; i<NSITE; i++) {
      // small_v
      double small_v = arma::sum(Z_run.row(i)-X.row(i)*beta_run-W_run.row(i)*lambda_run);
...
} // end Rcpp_jSDM_binomial_probit_rand_site_lv
...
#                                               Y=Y, X=X, 
#                                               beta_start=matrix(0,np,nsp), V_beta=diag(rep(100,np)), mu_beta = rep(0,np),
#                                               lambda_start=lambda_start, V_lambda=diag(rep(10,nl)), mu_lambda = rep(0,nl),
