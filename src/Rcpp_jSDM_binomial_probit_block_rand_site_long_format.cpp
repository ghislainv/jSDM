...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_binomial_probit_block_rand_site_long_format(
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++) {
      ...
      // small_v
      arma::vec small_v = inv(V_beta)*mu_beta + X.rows(rowId_sp[j]).t()*(Z_run(rowId_sp[j]) - alpha_run(Id_site(rowId_sp[j])));
...
} // end Rcpp_jSDM_binomial_probit_block_rand_site_long_format
...
# ngibbs <- nsamp+nburn
# T1 <- Sys.time()
# mod <- Rcpp_jSDM_binomial_probit_block_rand_site_long_format(
...
#   mu_beta = rep(0,np), alpha_start=rep(0,nsite),
...
# plot(alpha.target,summary(MCMC_alpha)[[1]][,"Mean"], xlab="obs", ylab ="fitted", main="Site effect alpha")
# abline(a=0,b=1,col='red')
# ## V_alpha
...
# abline(a=0,b=1,col='red')
# MCMC_Deviance <- coda::mcmc(mod$Deviance, start=nburn+1, end=ngibbs, thin=nthin)
# ## Deviance
# plot(MCMC_Deviance)
# mean(mod$Deviance)
*/
