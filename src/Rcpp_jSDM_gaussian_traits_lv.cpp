...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_gaussian_traits_lv(const int ngibbs,const int nthin, const int nburn, 
                                        const arma::mat &Y, 
                                        const arma::mat &X,
                                        const arma::mat &Tr,
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++){
      ...
      // Loop on species
      // small_v
      arma::vec small_v = inv_Vbeta*mu_beta_run.row(j).as_col() + X.t()*(Y.col(j)-W_run*lambda_run.col(j))/V_run;
      // big_V
...
} // end Rcpp_jSDM_gaussian_traits_lv
