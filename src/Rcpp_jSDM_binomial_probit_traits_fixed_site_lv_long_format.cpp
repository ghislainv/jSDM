...
Rcpp::List Rcpp_jSDM_binomial_probit_traits_fixed_site_lv_long_format(
  ...
  // probit(theta_ij) = X_i*beta_j + alpha_i + W_i*lambda_j + D_ij*gamma
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int i=0; i<NSITE; i++) {
      ...
      // big_V
      arma::mat big_V = inv(inv_VW +
        lambda_run.cols(Id_sp(rowId_site[i]))*lambda_run.cols(Id_sp(rowId_site[i])).t());
    ...
    for (int j=0; j<NSP; j++) {
      ...
      // small_v
      arma::vec small_v = inv_Vbeta*mu_beta +
        X.rows(rowId_sp[j]).t()*(Z_run(rowId_sp[j]) -
...
} // end Rcpp_jSDM_binomial_probit_traits_fixed_site_lv_long_format
...
# alpha.target[1] <- 0
# ## probit_theta
# probit_theta <- c(X %*% beta.target + W %*% lambda.target) + as.matrix(D) %*% gamma.target + rep(alpha.target, nsp)
...
# ngibbs <- nsamp+nburn
# T1 <- Sys.time()
# mod <- Rcpp_jSDM_binomial_probit_traits_fixed_site_lv_long_format(
...
# # Result analysis
# # ===================================================
# pdf(file="~/Documents/jSDM.test/results_binomial_probit_traits_fixed_site_lv_long_format.pdf")
# # Parameter estimates
# par(mfrow=c(1,2))
# hist(probit_theta)
# ## theta
