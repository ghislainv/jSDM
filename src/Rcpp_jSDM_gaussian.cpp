...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_gaussian(const int ngibbs,const int nthin, const int nburn, 
                                     const arma::mat &Y, 
                                     const arma::mat &X,
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++){
      // small_v
      arma::vec small_v = inv_Vbeta*mu_beta + X.t()*Y.col(j)/V_run;
      // big_V
