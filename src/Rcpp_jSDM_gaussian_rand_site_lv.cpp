...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_gaussian_rand_site_lv(const int ngibbs,const int nthin, const int nburn, 
                                           const arma::mat &Y, 
                                           const arma::mat &X,
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int i=0; i<NSITE; i++) {
      ...
      // small_v
      double small_v2 = arma::sum((Y.row(i) - X.row(i)*beta_run - W_run.row(i)*lambda_run)*1.0/V_run);
    ...
    ////////////////////////////////////////////////
    // V_alpha
    double sum_alpha = arma::as_scalar(alpha_run.t()*alpha_run);
    double shape_posterior_Valpha = shape_Valpha + 0.5*NSITE;
    double rate_posterior_Valpha = rate_Valpha + 0.5*sum_alpha;
    
    V_alpha_run = rate_posterior_Valpha/gsl_ran_gamma_mt(s, shape_posterior_Valpha, 1.0);
    ...
    for (int j=0; j<NSP; j++){
      ...
      for (int l=0; l<NL; l++){
        if (l > j) {
          ...
          double big_V = arma::as_scalar(1.0/(1.0/V_lambda(l,l) + 
                                         W_run.col(l).t()*W_run.col(l)/V_run));
          if (l!=j){
            // Draw in the posterior distribution
      ...
      // small_v
      arma::vec small_v = inv_Vbeta*mu_beta + X.t()*(Y.col(j)-W_run*lambda_run.col(j) - alpha_run)/V_run;
      // big_V
    ...
    for (int i = 0; i < NSITE; i++ ) {
      for (int j = 0; j < NSP; j++ ) {
        // Y_hat_ij = X_i*beta_j 
        Y_hat(i,j) = arma::as_scalar(X.row(i)*beta_run.col(j) + W_run.row(i)*lambda_run.col(j) + alpha_run(i));
        /* log Likelihood */
...
} // end Rcpp_jSDM_gaussian_rand_site_lv
...
# # theta
# par(mfrow=c(1,2))
# plot(mu.target, mod$Y_pred,
# xlab="obs", ylab="fitted", main="mu")
# abline(a=0, b=1, col='red')
# plot(Y, matrix(rnorm(nsite*nsp, mod$Y_pred, sqrt(mean(mod$V))), nrow=nsite),
#                xlab="obs", ylab="fitted", main="Y")
# abline(a=0, b=1, col='red')
*/