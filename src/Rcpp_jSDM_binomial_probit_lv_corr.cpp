...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_binomial_probit_lv_corr(const int ngibbs,const int nthin,const int nburn, 
                                             const arma::umat &Y, 
                                             const arma::mat &X,
  ...
  // We enforce our choice of signs on factor loading lambda
  lambda_run.submat(0,NL,NL-1,NSP-1) = target_sign % arma::sign(lambda_run.col(jhat)) % lambda_run.submat(0,NL,NL-1,NSP-1).each_col();
  umat upper_indices = arma::ind2sub(size(NL,NL),arma::trimatu_ind(size(NL,NL),1));
  // We enforce our choice of signs on latent axes W
  W_run.submat(NL,0,NSITE-1,NL-1) = target_sign.t() % arma::sign(lambda_run.col(jhat)).t() % W_run.submat(NL,0,NSITE-1,NL-1).each_row();
  for (int k=0; k<upper_indices.n_cols; k++){
  lambda_run(upper_indices(0,k),upper_indices(1,k))=target_sign(upper_indices(0,k)) * arma::sign(lambda_run.at(upper_indices(0,k),jhat)) * lambda_run(upper_indices(0,k),upper_indices(1,k));
  W_run(upper_indices(1,k),upper_indices(0,k))=target_sign(upper_indices(0,k)) * arma::sign(lambda_run.at(upper_indices(0,k),jhat)) * W_run(upper_indices(1,k),upper_indices(0,k));
  } 
  // Z latent (nsite*nsp)
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++) {
      // small_v
      arma::vec small_v = inv_Vbeta*mu_beta + X.t()*(Z_run.col(j) - W_run*lambda_run.col(j));
      // big_V
      ...
      for (int l=0; l<NL; l++) {
        if (l > j) {
          ...
          lambda_prop(l) = 0.0; 
          // small_v
          double small_v = arma::as_scalar(1.0/(V_lambda(l,l))*mu_lambda(l) + W_run.col(l).t()*(Z_run.col(j)-X*beta_run.col(j)-W_run*lambda_prop));
          // big_V
        ...
        // if(j>=NL){
        //   lambda_run(l,j) = target_sign.at(l) * arma::sign(lambda_run.at(l,jhat)) * lambda_run(l,j);
        // }
      }
    }
    // We now enforce our choice of signs on factor loading lambda
    lambda_run.submat(0,NL,NL-1,NSP-1) = target_sign % arma::sign(lambda_run.col(jhat)) % lambda_run.submat(0,NL,NL-1,NSP-1).each_col();
    for (int k=0; k<upper_indices.n_cols; k++){
      lambda_run(upper_indices(0,k),upper_indices(1,k))=target_sign(upper_indices(0,k)) * arma::sign(lambda_run.at(upper_indices(0,k),jhat)) * lambda_run(upper_indices(0,k),upper_indices(1,k));
    } 
    ...
    for (int i=0; i<NSITE; i++) {
      ...
      // if(i>=NL){
      //   // We enforce our choice of signs on latent axes W
      //   W_run.row(i) = (target_sign % arma::sign(lambda_run.col(jhat)) % W_i).t();
      // }
    }
    // We now enforce our choice of signs on latent axes W
    W_run.submat(NL,0,NSITE-1,NL-1) = target_sign.t() % arma::sign(lambda_run.col(jhat)).t() % W_run.submat(NL,0,NSITE-1,NL-1).each_row();
    for (int k=0; k<upper_indices.n_cols; k++){
      W_run(upper_indices(1,k),upper_indices(0,k))=target_sign(upper_indices(0,k)) * arma::sign(lambda_run.at(upper_indices(0,k),jhat)) * W_run(upper_indices(1,k),upper_indices(0,k));
    } 
...
} // end Rcpp_jSDM_binomial_probit_lv_corr
...
# W <- cbind(rnorm(nsite,0,1),rnorm(nsite,0,1),rnorm(nsite,0,1))
# mat <- t(matrix(runif(nsp*nl,-2,2), byrow=TRUE, nrow=nsp))
# diag(mat) <- runif(nl,1,2)
...
#                                     seed=123, verbose=1)
# 
# coef_var <- matrix(0,nsp,nl)
# for(j in 1:nsp){
#   for(l in 1:nl){
#     if(l <= j){
#   abs_lambda_jl <- abs(mod$lambda[,j,l])
# coef_var[j,l]   <- mean(abs_lambda_jl)/sd(abs_lambda_jl)
#     }
#   }
# }
# jhat <- arrayInd(which.max(coef_var[(nl+1):nsp,]),dim(coef_var[(nl+1):nsp,]))[1] + nl
# 
# # ===================================================
# # Result analysis
# # ===================================================
...
# abline(a=0,b=1,col='red')
# RMSE <- sqrt(sum((probit_theta - mod$probit_theta_latent)^2))
# #
# # Call to C++ function
# # Iterations
# nsamp <- 5000
...
# nthin <- 5
# ngibbs <- nsamp+nburn
# mod_corr <- Rcpp_jSDM_binomial_probit_lv_corr(ngibbs=ngibbs, nthin=nthin, nburn=nburn,
#                                               Y=Y, X=X,
#                                               beta_start=matrix(0,np,nsp),
#                                               V_beta=diag(rep(1,np)), mu_beta=rep(0,np),
#                                               lambda_start=lambda_start,
#                                               V_lambda=diag(rep(1,nl)), mu_lambda = rep(0,nl),
#                                               W_start=matrix(0,nsite,nl), V_W=diag(rep(1,nl)),
#                                               seed=123, jhat=jhat-1,
#                                               target_sign=sign(colMeans(mod$lambda[,jhat,])),verbose=1)
# 
# # ===================================================
# # Result analysis
# # ===================================================
...
# mean_beta <- matrix(0,nsp,np)
# for (j in 1:nsp) {
#   mean_beta[j,] <-apply(mod_corr$beta[,j,],2,mean)
#   if(j<5){
#     for (p in 1:np) {
#       MCMC.betaj <- coda::mcmc(mod_corr$beta[,j,], start=nburn+1, end=ngibbs, thin=nthin)
#       summary(MCMC.betaj)
#       coda::traceplot(MCMC.betaj[,p])
#       coda::densplot(MCMC.betaj[,p], main = paste0("beta",p,j))
#       abline(v=beta.target[p,j],col='red')
...
# mean_lambda <- matrix(0,nsp,nl)
# for (j in 1:nsp) {
#   mean_lambda[j,] <- apply(mod_corr$lambda[,j,],2,mean)
#   if(j<5){
#     for (l in 1:nl) {
#       MCMC.lambdaj <- coda::mcmc(mod_corr$lambda[,j,], start=nburn+1, end=ngibbs, thin=nthin)
#       summary(MCMC.lambdaj)
#       coda::traceplot(MCMC.lambdaj[,l])
#       coda::densplot(MCMC.lambdaj[,l],main = paste0("lambda",l,j))
#       abline(v=lambda.target[l,j],col='red')
...
# # W latent variables
# par(mfrow=c(1,2),oma=c(1, 0, 1, 0))
# mean_W <- apply(mod_corr$W, c(2,3), mean)
# for(l in 1:nl){
#   plot(W[,l],mean_W[,l], main=paste0("W",l),xlab="obs", ylab= "fitted")
#   abline(a=0,b=1,col='red')
#   title("Variables latentes", outer=TRUE)
...
# 
# ## Deviance
# mean(mod_corr$Deviance)
# ## Prediction
# # theta
# plot(pnorm(probit_theta),mod_corr$theta_latent,xlab="obs", ylab="fitted",main="theta")
# abline(a=0,b=1,col='red')
# # probit_theta
# plot(probit_theta,mod_corr$probit_theta_latent,xlab="obs", ylab="fitted",main="probit(theta)")
# abline(a=0,b=1,col='red')
# # Z
# plot(Z_true,mod_corr$Z_latent, xlab="obs", ylab="fitted",main="Z_latent" )
# abline(a=0,b=1,col='red')
# RMSE_corr <- sqrt(sum((probit_theta - mod_corr$probit_theta_latent)^2))
*/ 