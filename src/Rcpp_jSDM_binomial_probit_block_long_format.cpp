...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_binomial_probit_block_long_format(
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++) {
      ...
      // small_v
      arma::vec small_v = inv(V_beta)*mu_beta + X.rows(rowId_sp[j]).t()*(Z_run(rowId_sp[j]));
...
} // end Rcpp_jSDM_binomial_probit_block_long_format
...
# ngibbs <- nsamp+nburn
# T1 <- Sys.time()
# mod <- Rcpp_jSDM_binomial_probit_block_long_format(
