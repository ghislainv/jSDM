...
// [[Rcpp::export]]
Rcpp::List Rcpp_jSDM_binomial_probit_block_lv_long_format(
  ...
  for (int g=0; g < NGIBBS; g++) {
    ...
    for (int j=0; j<NSP; j++) {
      ...
      // small_v
      arma::vec small_v = inv(V_param)*mu_param + data.rows(rowId_sp[j]).t()*(Z_run(rowId_sp[j]));
    ...
    for ( int n = 0; n < NOBS; n++ ) {
      ...
      probit_theta_run(n) = arma::as_scalar(data.row(n)*param_run.col(Id_sp(n)));
...
} // end Rcpp_jSDM_binomial_probit_block_lv_long_format
...
# ngibbs <- nsamp+nburn
# T1 <- Sys.time()
# mod <- Rcpp_jSDM_binomial_probit_block_lv_long_format(
